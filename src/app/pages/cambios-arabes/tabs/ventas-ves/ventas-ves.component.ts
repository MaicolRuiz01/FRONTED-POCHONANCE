...
export class VentasVesComponent implements OnInit {
  ...
  submit(): void {
    ...
    if (v.asignadoA === 'cuenta' && !v.cuentaCopId) return this.notificationService.warn('Seleccione una Cuenta COP');
    if (v.asignadoA === 'cliente' && !v.clienteId) return this.notificationService.warn('Seleccione un Cliente');
    if (v.asignadoA === 'proveedor' && !v.proveedorId) return this.notificationService.warn('Seleccione un Proveedor');
